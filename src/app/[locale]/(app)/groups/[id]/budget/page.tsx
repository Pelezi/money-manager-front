...
export default function GroupBudgetPage() {
  ...
  const getActualAmount = (subcategoryId: number, month: number) => {
    // Sum all transactions for this subcategory/month (may be multiple entries if group has multiple users)
                    ...
                    <tr key={subcategory.id} className="border-t border-gray-100 dark:border-gray-700 hover:bg-gray-50 dark:hover:bg-gray-700">
                      ...
                      {MONTHS.map((_, index) => {
                        ...
                        return (
                          <td
                            key={index}
                            className={`px-1.5 py-1.5 text-center border-r border-gray-200 dark:border-gray-600 ${status} ${canManage ? 'cursor-pointer' : ''}`}
                            onClick={() => handleCellClick(subcategory.id, month)}
                          ...
                          >
                            {isEditing ? (
                              ...
                              <div className={canManage ? 'hover:bg-opacity-80' : ''}>
